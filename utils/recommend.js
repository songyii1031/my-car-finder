...
const isHighBudget = (budget) => budgetLevel[budget] >= 3;
// 예산이 낮은지 확인 (2~4천만원 이하)
const isLowBudget = (budget) => budgetLevel[budget] <= 2;
// 예산이 매우 높은지 확인 (7천만원 이상)
const isVeryHighBudget = (budget) => budgetLevel[budget] >= 4;
...
export function recommendCars(input) {
    ...
    // 선호도 확인 헬퍼
    const hasPreference = (pref) => preferences.includes(pref);
    // 1. 스포티 성향 처리
